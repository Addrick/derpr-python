...
def _resolver(reply):
    """A `stream_resolve_deferral` stub that records calls and yields one reply.

    Bound onto the fake ChatSystem after construction so the recorded calls
    live on the namespace itself (`chat_system.resolved`).
    """
    calls = []

    def stream_resolve_deferral(token, *, kind, status, result, note=None):
        calls.append({"token": token, "kind": kind, "status": status,
                      "result": result, "note": note})

        async def _gen():
            yield DoneEvent(text=reply,
                            response_type=ResponseType.DEV_COMMAND)
        return _gen()

    stream_resolve_deferral.calls = calls
    return stream_resolve_deferral
...
def _wired(runner=None):
    ...
    chat_system = SimpleNamespace(
        stream_resolve_deferral=_resolver("newmodel landed"),
        personas={},
        visible_personas={},
    ...
    chat_system.resolved = chat_system.stream_resolve_deferral.calls
...
def test_ping_reaches_the_persona_through_the_real_route(route_configured):
    ...
    assert runner.calls == [["/usr/local/sbin/derpr-model-install", "status",
                            "newmodel-abc123"]]
    # The job id addresses the park; the conversation comes off the row, so
    # nothing on this call names a persona, a channel or a user.
    call = chat_system.resolved[0]
    assert call["token"] == "newmodel-abc123"
    assert call["kind"] == "node_job"
    assert call["result"]["unit"] == "koboldcpp-newmodel.service"
...
def test_unwired_adapter_leaves_the_callback_path_gated(route_configured):
    chat_system = SimpleNamespace(
        stream_resolve_deferral=_resolver("unused"),
